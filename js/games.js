import { mediaTitle, t } from './i18n.js';
...
export function renderGames() {
  ...
  const list = games.map((g, i) => ({...g, i})).filter(g =>
    (!q || [mediaTitle(g,'es'), mediaTitle(g,'en'), g.developer, g.platform].join(' ').toLowerCase().includes(q)) &&
    (status === 'all' || (g.status || 'Backlog') === status));
  if (sort === 'title') list.sort((a, b) => mediaTitle(a).localeCompare(mediaTitle(b)));
  ...
  el('gamesGrid').innerHTML = list.map(g => {
    ...
    return `<div class="m-card" ${_authed ? `data-edit-type="game" data-edit-idx="${g.i}"` : ''}>
      <div class="m-card-top"><div class="m-card-title">${esc(mediaTitle(g))}</div><div class="m-card-rating" style="color:${g.myRating == null ? 'var(--text-muted)' : ratingColor(g.myRating, 10)}">${g.myRating == null ? '—' : g.myRating + '/10'}</div></div>
      <div class="m-card-meta">${esc(t(g.status || 'Backlog'))}${meta.length ? ' · ' + meta.map(x=>esc(t(x))).join(' · ') : ''}</div>
...
export function openGameModal(idx = null) {
  ...
  if (!g) return;
  const fields = [['title','Title · Español','text'], ['titleEn','Title · English','text'], ['platform','Platform','text'], ['developer','Developer','text'],
  ...
  el('mod').innerHTML = `<h2>${idx == null ? 'Add' : 'Edit'} Game</h2><form id="gameForm">
    <div class="game-fields">${fields.map(([key, label, type]) => `<div class="fg"><label for="game-${key}">${label}</label><input id="game-${key}" name="${key}" type="${type}" value="${esc(key==='title'?mediaTitle(g,'es'):key==='titleEn'?mediaTitle(g,'en'):g[key])}" ${bounds[key] || ''} ${key === 'title' || key === 'titleEn' ? 'required' : ''} ${key === 'platform' ? 'list="gamePlatforms"' : ''}></div>`).join('')}</div>
    ...
    <div class="fg"><label for="game-status">Status</label><select id="game-status" name="status">${GAME_STATUSES.map(s => `<option value="${s}" ${s === (g.status || 'Backlog') ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
  ...
  el('gameDelete')?.addEventListener('click', () => {
    if (!_authed || !confirm(t('Delete this item?'))) return;
  ...
  el('gameForm').addEventListener('submit', event => {
    ...
    next.translations={...g.translations,es:{...g.translations?.es,title:next.title},en:{...g.translations?.en,title:next.titleEn}};
    delete next.titleEn;
