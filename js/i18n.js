...
function formatTranslation(s){
  for(const {pair,keys,re} of FORMAT_RULES){
    ...
    if(match){
      const values=Object.fromEntries(keys.map((k,i)=>[k,match[i+1]]));
      return pair[language()==='es'?1:0].replace(/\{(\w+)\}/g,(_,k)=>values[k]);
    }
  }
  return null;
}
